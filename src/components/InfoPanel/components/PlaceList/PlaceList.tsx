...
function PlaceList({ data, isLoading, error }: TPlaceListProps) {
  if (isLoading)
    return (
      <ul className="flex flex-row lg:flex-col gap-4 flex-1 h-full overflow-y-scroll pr-2">
        {[0, 1, 2].map((_, i) => (
  ...
  return (
    <ul className="flex flex-row lg:flex-col gap-4 flex-1 overflow-x-scroll lg:overflow-y-scroll pr-2 h-48 lg:h-full items-stretch">
      {!data || data.length === 0 ? (
